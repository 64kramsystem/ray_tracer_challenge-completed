...
pub fn shape_accessors_derive(input: TokenStream) -> TokenStream {
    ...
    let gen = quote! {
        ...
        use std::sync::Mutex as ShapeAccessorsMutex;
        ...
        impl Shape for #name {
            ...
            fn parent(&self) -> Option<ShapeAccessorsArc<ShapeAccessorsMutex<dyn ShapeAccessorsShape>>> {
                ShapeAccessorsWeak::upgrade(&self.parent)
            }

            fn parent_mut(&mut self) -> &mut ShapeAccessorsWeak<ShapeAccessorsMutex<dyn ShapeAccessorsShape>> {
                &mut self.parent
            }
