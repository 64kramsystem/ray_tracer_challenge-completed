...
const WALL_SIZE: usize = 100;
const EYE_Z: f64 = -50.0;
const WALL_Z: f64 = 50.0;

fn prepare_objects() -> (Sphere, PointLight) {
...
fn compute_color(
    ...
    let eye_ray_direction =
        Tuple::vector(interface_x as f64, interface_y as f64, WALL_Z - EYE_Z).normalize();
...
fn compute(sphere: &Sphere, light: &PointLight) -> Vec<[Color; WALL_SIZE]> {
    ...
    (-origin_y..origin_y)
        ...
        .for_each(|(buffer_y, interface_y)| {
            ...
            for (buffer_x, interface_x) in (-origin_x..origin_x).enumerate() {
                if let Some(light_color) = compute_color(interface_x, interface_y, sphere, light) {
...
fn launch_interface(pixels_buffer: &Vec<[Color; WALL_SIZE]>) {
    let mut interface = Sdl2Interface::init(file!(), WALL_SIZE as u16, WALL_SIZE as u16, (0, 0));
...
pub fn practice() {
    let (sphere, light) = prepare_objects();

    let pixels_buffer = compute(&sphere, &light);

    launch_interface(&pixels_buffer);
}
